...
# This is an example of a class which implements AbstractFrameCachePreprocessor.
...
class PCAPreprocessor(preprocess.AbstractFrameCachePreprocessor):
    def __init__(self, db_name=None, target_columns=None, number_components=None):
        ...
        self.target_columns = set(target_columns) # These are the columns we would like to include in the analysis.
        self.number_components = number_components
        self.cache_columns = ['pca_comp_{0}'.format(i+1) for i in range(number_components)] # We must override this
                                                                        # to use the caching logic in the base class
    ...
    def _transform(self, data):
        ...
        pca_transformed_data = pca.fit_transform(pca_data.values) # The PCA occurs here; the model is not kept around.
        data = deepcopy(data)
